...
export default function Recipients() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      setForm(empty);
      setSuccess("Candidate registered successfully on the waitlist!");
      load();
    } catch (err) {
      setError(err.response?.data?.message || "Could not save candidate");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="page">
      <div className="page-heading">
        <div>
          <span className="eyebrow">UC2 · FR2 · FR8</span>
          <h2>Recipient waitlist</h2>
          <p>Register candidates and maintain a dynamic urgency-based waitlist.</p>
        </div>
      </div>
      ...
      <div className="two-col form-layout">
        ...
        <div className="panel">
          <div className="panel-title">
            ...
            <span>{rows.filter(r => r.status === "Active").length} active candidates</span>
          </div>
              ...
              <tbody>
                {rows.map(r => (
                  <tr key={r._id || r.candidateId}>
                    ...
                    <td>{r.candidateId}<br/><small>{r.name}</small></td>
